...
SELECT cc.s_id, cc.cart_date, c.* FROM cart_courses cc
		inner join course c on cc.c_no = c.c_no
		WHERE cc.s_id='sss01'
		order by cc.s_id, cc.cart_date;
		
		
		SELECT ccc.s_id, ccc.cart_date, c.* FROM cart_courses ccc
		inner join course c on ccc.c_no = c.c_no
		WHERE ccc.s_id='sss01'
		order by ccc.s_id, ccc.cart_date;
		
		
		
		SELECT ccc.s_id, ccc.cart_date, c.* FROM cart_courses ccc inner join course c on ccc.c_no = 
c.c_no WHERE ccc.s_id='sss01' order by ccc.s_id, ccc.cart_date ;
...
SELECT at_no, att.s_id, the_time, att.as_no, ass.as_status, ccc.c_no, ccc.c_name, sg.gd_name, sss.s_name
		...
		left outer join course ccc on ccc.c_no = cr.reg_c_no
		where att.s_id = 'sss01' and date_format(att.the_time, '%Y-%m') = '2017-07' and att.as_no != 5
